            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑盘点</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yuangongId" name="yuangongId" type="hidden">
                                    ...
                                    <div class="form-group  col-md-6">
                                        <label>备注</label>
                                        <input id="pandianOrderContentupload" name="file" type="file">
                                        <script id="pandianOrderContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var pandianOrderContentUe = UE.getEditor('pandianOrderContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="pandianOrderContent" id="pandianOrderContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#pandianOrderContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('pandianOrderContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("pandianOrder/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addpandianOrder');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.pandianOrderContent != null && ruleForm.pandianOrderContent != 'null' && ruleForm.pandianOrderContent != '' && $("#pandianOrderContentupload").length>0) {

            var pandianOrderContentUeditor = UE.getEditor('pandianOrderContentEditor');
            pandianOrderContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.pandianOrderContent != null){
                    mes = ''+ ruleForm.pandianOrderContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                pandianOrderContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#pandianOrderContentupload").length>0) {
            var pandianOrderContentEditor = UE.getEditor('pandianOrderContentEditor');
            if (pandianOrderContentEditor.hasContents()) {
                $('#pandianOrderContent-input').attr('value', pandianOrderContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yuangongId: "required",
                pandianOrderName: "required",
                insertTime: "required",
                pandianOrderContent: "required",
            },
            messages: {
                yuangongId: "操作人不能为空",
                pandianOrderName: "盘点名不能为空",
                insertTime: "盘点时间不能为空",
                pandianOrderContent: "备注不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addpandianOrder = window.sessionStorage.getItem("addpandianOrder");
        if (addpandianOrder != null && addpandianOrder != "" && addpandianOrder != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("pandianOrder/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
