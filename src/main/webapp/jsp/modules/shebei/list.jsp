...
<!--/Pre Loader -->
                            ...
                            <div class="table-responsive mb-3">
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        设备数量
                                        <div class="layui-input-inline">
                                            <input type="text" id="shebeiNumberStartSearch" style="width: 100px;" class="form-control form-control-sm" onchange="shebeiNumberChickValue(this)"
                                                   placeholder="开始" aria-controls="tableId">
                                        </div>
                                    </div>
                                    <div class="layui-inline" style="margin-bottom: 10px;">
                                        <div class="layui-input-inline">
                                            <input type="text" id="shebeiNumberEndSearch" style="width: 100px;" class="form-control form-control-sm" onchange="shebeiNumberChickValue(this)"
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //设备数量
            var shebeiNumberCell = document.createElement('td');
            shebeiNumberCell.innerHTML = item.shebeiNumber;
            row.appendChild(shebeiNumberCell);
            ...
            row.appendChild(insertTimeCell);
                ...
                //设备详情
            var shebeiContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.shebeiContent && item.shebeiContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.shebeiContent.indexOf('<img');
                var endIndex = item.shebeiContent.indexOf('>');
                    shebeiContentCell.innerHTML = item.shebeiContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.shebeiContent != null && item.shebeiContent != "" && item.shebeiContent.length >= 11) {
                        shebeiContentCell.innerHTML = item.shebeiContent.substring(0, 10) + "...";
                } else {
                        shebeiContentCell.innerHTML = item.shebeiContent;
                }
            }
            row.appendChild(shebeiContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("shebei/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addshebei", "addshebei");
            window.location.href = "add-or-update.jsp"
        }
