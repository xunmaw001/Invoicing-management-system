                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="huiyuanId" name="huiyuanId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>单号</label>
                                        <input id="goodsOutDanhaoNumber" name="goodsOutDanhaoNumber" class="form-control"
                                               placeholder="单号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>售卖数量</label>
                                        <input id="goodsOutNumber" name="goodsOutNumber" class="form-control"
                                               onchange="goodsOutNumberChickValue(this)"  placeholder="售卖数量">
                                    </div>

                                    <div class="form-group  col-md-6">
                                        <label>备注</label>
                                        <input id="goodsOutContentupload" name="file" type="file">
                                        <script id="goodsOutContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var goodsOutContentUe = UE.getEditor('goodsOutContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="goodsOutContent" id="goodsOutContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#goodsOutContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('goodsOutContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.goodsOutContent != null && ruleForm.goodsOutContent != 'null' && ruleForm.goodsOutContent != '' && $("#goodsOutContentupload").length>0) {

            var goodsOutContentUeditor = UE.getEditor('goodsOutContentEditor');
            goodsOutContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.goodsOutContent != null){
                    mes = ''+ ruleForm.goodsOutContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                goodsOutContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#goodsOutContentupload").length>0) {
            var goodsOutContentEditor = UE.getEditor('goodsOutContentEditor');
            if (goodsOutContentEditor.hasContents()) {
                $('#goodsOutContent-input').attr('value', goodsOutContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                yuangongId: "required",
                huiyuanId: "required",
                goodsOutDanhaoNumber: "required",
                goodsOutNumber: "required",
                insertTime: "required",
                goodsOutContent: "required",
            },
            messages: {
                goodsId: "商品不能为空",
                yuangongId: "操作人不能为空",
                huiyuanId: "操作人不能为空",
                goodsOutDanhaoNumber: "单号不能为空",
                goodsOutNumber: "售卖数量不能为空",
                insertTime: "售卖时间不能为空",
                goodsOutContent: "备注不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addgoodsOut != null && addgoodsOut != "" && addgoodsOut != "null") {
            //注册表单验证
            $(validform());
            $("#goodsOutDanhaoNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置单号
            $('#submitBtn').text('新增');
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#goodsOutDanhaoNumber").val(ruleForm.goodsOutDanhaoNumber);
        $("#goodsOutNumber").val(ruleForm.goodsOutNumber);
        $("#insertTime-input").val(ruleForm.insertTime);
        $("#goodsOutContent").val(ruleForm.goodsOutContent);
